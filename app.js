...
function setMapTool(tool){
  ...
  byId('map-workspace').classList.toggle('waypoint-mode',tool==='waypoint'||tool==='measure');
  text('map-hint',tool==='measure'?'Click to add measurement points · click CLEAR MEASURE to restart':'Click the map to place a waypoint');
  byId('map-hint').classList.toggle('hidden',!['waypoint','measure'].includes(tool));
}
...
function updateMapDetails(map){
  ...
  text('map-detail-contacts',`${tracks.size} TRACK${tracks.size===1?'':'S'}`);text('map-detail-geofences',`${geofences.length} ZONE${geofences.length===1?'':'S'}`);text('map-detail-route',`${missionWaypoints.length} WAYPOINT${missionWaypoints.length===1?'':'S'}`);text('map-detail-measurement',measurementPoints.length>1?formatMapDistance(measurementDistance(measurementPoints)):'NOT SET');
}
function formatMapDistance(distance){return distance>=1000?`${(distance/1000).toFixed(distance>=10000?0:2)} km`:`${Math.round(distance)} m`}
function measurementDistance(points){return points.slice(1).reduce((total,point,index)=>total+haversine(points[index],point),0)}
function clearMeasurement(){measurementPoints=[];measurementCursor=null;text('plot-state','MEASUREMENT CLEARED');drawTacticalPlot()}
function toggleRangeRings(){showRangeRings=!showRangeRings;const button=byId('map-range-rings');if(button){button.classList.toggle('active',showRangeRings);button.setAttribute('aria-pressed',String(showRangeRings))}drawTacticalPlot()}
async function toggleMapFullscreen(){const stage=byId('map-workspace');try{if(document.fullscreenElement)await document.exitFullscreen();else await stage.requestFullscreen()}catch(_){text('plot-state','FULL SCREEN UNAVAILABLE')}}
function drawMeasurement(map){
  const preview=activeMapTool==='measure'&&measurementCursor?[...measurementPoints,measurementCursor]:measurementPoints;if(!preview.length)return;
  const{ctx,plot}=map;ctx.save();ctx.strokeStyle='#8fe0ff';ctx.fillStyle='#d7f4ff';ctx.lineWidth=2;ctx.setLineDash([6,4]);ctx.beginPath();preview.forEach((point,index)=>{const[x,y]=plot([point.latitude,point.longitude]);index?ctx.lineTo(x,y):ctx.moveTo(x,y)});if(preview.length>1)ctx.stroke();ctx.setLineDash([]);
  preview.forEach((point,index)=>{const[x,y]=plot([point.latitude,point.longitude]);ctx.beginPath();ctx.arc(x,y,index===0?5:4,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#081218';ctx.lineWidth=1.5;ctx.stroke()});
  if(preview.length>1){const end=preview.at(-1),[x,y]=plot([end.latitude,end.longitude]),label=`MEASURE ${formatMapDistance(measurementDistance(preview))}`;ctx.font='700 9px "Cascadia Mono",Consolas,monospace';const labelWidth=ctx.measureText(label).width+12;ctx.fillStyle='#071218e8';ctx.fillRect(x+9,y+8,labelWidth,18);ctx.strokeStyle='#70c7e7';ctx.strokeRect(x+9,y+8,labelWidth,18);ctx.fillStyle='#d7f4ff';ctx.fillText(label,x+15,y+20)}
  ctx.restore();
}
...
function drawTacticalPlotNow(){
  ...
  if(latestTelemetry){const[x,y]=plot([latestTelemetry.latitude,latestTelemetry.longitude]),pulse=(Math.sin(performance.now()/320)+1)/2;if(showRangeRings){[100,250,500].forEach((distance,index)=>{const radius=distance*scale;if(radius<10||radius>Math.max(width,height)*1.25)return;ctx.strokeStyle=index===2?'#66b6d855':'#66b6d888';ctx.lineWidth=1;ctx.setLineDash([5,5]);ctx.beginPath();ctx.arc(x,y,radius,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#b9e6f6';ctx.font='700 8px "Cascadia Mono",Consolas,monospace';ctx.fillText(`${distance} m`,x+radius+4,y-4)});const heading=Number(latestTelemetry.heading_deg);if(Number.isFinite(heading)){const radians=heading*Math.PI/180;ctx.strokeStyle='#7fd3f1bb';ctx.lineWidth=1.25;ctx.setLineDash([4,4]);ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+Math.sin(radians)*Math.min(500*scale,240),y-Math.cos(radians)*Math.min(500*scale,240));ctx.stroke();ctx.setLineDash([])}}ctx.strokeStyle=`rgba(57,168,232,${.55-pulse*.25})`;ctx.lineWidth=1;ctx.beginPath();ctx.arc(x,y,17+pulse*6,0,Math.PI*2);ctx.stroke();ctx.save();ctx.translate(x,y);ctx.rotate((latestTelemetry.heading_deg||0)*Math.PI/180);ctx.fillStyle='#39a8e8';ctx.strokeStyle='#dff4ff';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,-13);ctx.lineTo(8,10);ctx.lineTo(0,6);ctx.lineTo(-8,10);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore()}
...
function addWaypoint(event){
  ...
  if(activeMapTool==='measure'){if(measurementPoints.length>=8)measurementPoints=[];measurementPoints.push(map.unplot(pointerX,pointerY));measurementCursor=null;text('plot-state',measurementPoints.length===1?'MEASURE START SET':`MEASURE ${formatMapDistance(measurementDistance(measurementPoints))}`);drawTacticalPlot();return}
...
function updateCursor(event){if(mapDrag)return;const point=cursorCoordinate(event);lastMapCursor=point;if(activeMapTool==='measure'){measurementCursor=point;drawTacticalPlot()}text('cursor-coordinate',`${point.latitude.toFixed(5)}, ${point.longitude.toFixed(5)}`)}
...
function bindControls(){
  ...
  byId('street-view-button').addEventListener('click',openStreetView);byId('map-range-rings').addEventListener('click',toggleRangeRings);byId('map-fullscreen').addEventListener('click',toggleMapFullscreen);byId('map-measure-clear').addEventListener('click',clearMeasurement);byId('camera-source-form').addEventListener('submit',saveCameraSource);byId('asset-registration-form').addEventListener('submit',registerOperatorAsset);
  ...
  document.addEventListener('fullscreenchange',()=>{const cameraButton=byId('camera-fullscreen'),mapButton=byId('map-fullscreen');if(cameraButton)cameraButton.textContent=byId('camera-stage')===document.fullscreenElement?'EXIT FULL SCREEN':'FULL SCREEN';if(mapButton)mapButton.classList.toggle('active',byId('map-workspace')===document.fullscreenElement)});
